...
@CapabilityDescription("Fetches messages from an Amazon Simple Queuing Service Queue." +
		...
		"'Endpoint Override URL' property for use with other SQS-compatible endpoints.\n")
@WritesAttributes({
...
public class GetSQS extends AbstractSQSProcessor {
	...
	public static final PropertyDescriptor RECEIVE_MSG_WAIT_TIME = new PropertyDescriptor.Builder()
			...
			.description(
					"The maximum amount of time to wait on a long polling receive call. Setting this to a value of 1 second or greater will "
							+ "reduce the number of SQS requests and decrease fetch latency at the cost of a constantly active thread.")
			.expressionLanguageSupported(ExpressionLanguageScope.NONE)
			.required(true)
			.defaultValue("0 sec")
			.addValidator(StandardValidators.createTimePeriodValidator(0, TimeUnit.SECONDS, 20, TimeUnit.SECONDS)) // 20
																													// seconds
																													// is
																													// the
																													// maximum
	...
	public static final List<PropertyDescriptor> properties = Collections.unmodifiableList(
			Arrays.asList(DYNAMIC_QUEUE_URL, AUTO_DELETE, ACCESS_KEY, SECRET_KEY, CREDENTIALS_FILE,
					AWS_CREDENTIALS_PROVIDER_SERVICE, REGION, BATCH_SIZE, TIMEOUT, SSL_CONTEXT_SERVICE,
					ENDPOINT_OVERRIDE,
